...
/**
 * The class K2KC45NodeModel extends the NodeModel class and allows the execution of the node.
 * 
 *
 * @author Mirko Russo
 */
public class K2KC45NodeModel extends NodeModel{  
    ...
    private KEELAlgorithm k = new KEELAlgorithmC45();
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec) throws IOException {
    	...
    	(new File(mainDirectory + KEELAlgorithmC45.DATA_FILES_DIRECTORY)).mkdir();
    	(new File(mainDirectory + KEELAlgorithmC45.RESULTS_FILES_DIRECTORY)).mkdir();

    	KNIME2KEEL knimeToKeel = new KNIME2KEELC45(datasetName, mainDirectory, KEELAlgorithmC45.DATA_FILES_DIRECTORY, m_column.getStringValue());
    	knimeToKeel.configFileCreator(KEELAlgorithmC45.ALGORITHM_NAME, KEELAlgorithmC45.RESULTS_FILES_DIRECTORY, k.getKEELSettings());
    	knimeToKeel.dataFileCreator(trainingFileName, inData[0]);
    	knimeToKeel.dataFileCreator(testFileName, inData[1]);
		
    	k.runKEELAlgorithm(mainDirectory + KEELAlgorithmC45.CONFIG_FILE_NAME);
    	
    	BufferedDataTable[] outData = new BufferedDataTable[2];
    	KEEL2KNIME keelToKnime = new KEEL2KNIMEC45(mainDirectory, KEELAlgorithmC45.RESULTS_FILES_DIRECTORY, predictionColumn, exec);
    	outData[0] = keelToKnime.outDataCreator(inData[0], keelToKnime.resultsExtractor(KEELAlgorithmC45.RESULTS_TRAIN_FILE_NAME));
    	outData[1] = keelToKnime.outDataCreator(inData[1], keelToKnime.resultsExtractor(KEELAlgorithmC45.RESULTS_TEST_FILE_NAME));
