...
import org.mirkorusso.K2KFURIAC.K2K.KNIME2KEEL;


/**
 * The class K2KFURIACNodeModel extends the NodeModel class and allows the execution of the node.
 * 
 *
 * @author Mirko Russo
 */
public class K2KFURIACNodeModel extends NodeModel{  
    ...
    private KEELAlgorithm k = new KEELAlgorithmFURIAC();
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec) throws IOException {
    	...
    	(new File(mainDirectory + KEELAlgorithmFURIAC.DATA_FILES_DIRECTORY)).mkdir();
    	(new File(mainDirectory + KEELAlgorithmFURIAC.RESULTS_FILES_DIRECTORY)).mkdir();

    	KNIME2KEEL knimeToKeel = new KNIME2KEELFURIAC(datasetName, mainDirectory, KEELAlgorithmFURIAC.DATA_FILES_DIRECTORY, m_column.getStringValue());
    	knimeToKeel.configFileCreator(KEELAlgorithmFURIAC.ALGORITHM_NAME, KEELAlgorithmFURIAC.RESULTS_FILES_DIRECTORY, k.getKEELSettings());
    	knimeToKeel.dataFileCreator(trainingFileName, inData[0]);
    	knimeToKeel.dataFileCreator(testFileName, inData[1]);
		
    	k.runKEELAlgorithm(mainDirectory + KEELAlgorithmFURIAC.CONFIG_FILE_NAME);
    	
    	BufferedDataTable[] outData = new BufferedDataTable[2];
    	KEEL2KNIMEFURIAC keelToKnime = new KEEL2KNIMEFURIAC(mainDirectory, KEELAlgorithmFURIAC.RESULTS_FILES_DIRECTORY, predictionColumn, exec);
    	outData[0] = keelToKnime.outDataCreator(inData[0], keelToKnime.resultsExtractor(KEELAlgorithmFURIAC.RESULTS_TRAIN_FILE_NAME));
    	outData[1] = keelToKnime.outDataCreator(inData[1], keelToKnime.resultsExtractor(KEELAlgorithmFURIAC.RESULTS_TEST_FILE_NAME));
    	model = keelToKnime.modelExtractor("result.txt");
    	// files deletion
    	System.gc();
